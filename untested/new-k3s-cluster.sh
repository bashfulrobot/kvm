...
#HOSTS=(k8c k81 k82 k83)
# Primary controller must ALWAYS be the first in the array.
HOSTS=(k3c k3n1 k3n2 k3n3)
...
do
 ...
 nano ${SCRIPT}
printYellow "file saved - please run the script again."
exit 0
 ;;
     [nN][oO]|[nN])
...
# Give time to copy to the clipboard
sleep 5

# create K3SUP cluster

printTitle "creating k3s cluster"
# Install k3sup if it does not exist
if test -f "/usr/local/bin/k3sup"; then
    printYellow "installing k3sup"
    curl -sLS https://get.k3sup.dev | sh
    sudo install k3sup /usr/local/bin/
fi


# Create primary from first in host array
if [ "$HOST" = "${HOSTS[0]}" ]; then
    printYellow "creating k3s primary"
    K3S_PRIMARY_IP="${MYIP2}"
    k3sup install --ip "${K3S_PRIMARY_IP}" --user "${USERID}" --ssh-key $HOME/.ssh/id_rsa --cluster --context "${CONTEXT}" --merge --local-path $HOME/.kube/config --print-config
else
    printYellow "creating k3s cluser nodes"
    k3sup join --ip "${MYIP2}" --server-ip "${K3S_PRIMARY_IP}" --user "${USERID}" --server
fi
...
scp  $HOME/.kube/config dustin@192.168.169.2:~/.kube/clusters/${CONTEXT}-kubeconfig
